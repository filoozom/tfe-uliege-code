...
const parseListenType = (type, listen) => {
  const [host, port] = listen.split(":");
  switch (type) {
    case "tcp":
      return ["", "ip4", host, "tcp", port].join("/");
    case "wss":
      return ["", "ip4", host, "tcp", port, "wss"].join("/");
  }
};
const parseListen = (options) => {
  return [
    parseListenType("tcp", options.listenTcp),
    parseListenType("wss", options.listenWss),
  ].filter(Boolean);
};
...
const createNode = async (options) => {
  ...
  const listen = parseListen(options);
  const transport = [
